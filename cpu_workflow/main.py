...
def _callback(payload: dict[str, Any], body: dict[str, Any]) -> None:
    ...
    response = requests.post(callback_url, json=body, headers={"Authorization": f"Bearer {anon_key}", "apikey": anon_key, "Content-Type": "application/json", "User-Agent": "EVS-CPU-Compositor/2.0"}, timeout=45)
    if not response.ok:
...
def _rounded_card(img: Image.Image, max_w: int, max_h: int, radius: int = 34, pad: int = 16) -> Image.Image:
    ...
    card = Image.new("RGBA", shadow.size, (0,0,0,0)); card.alpha_composite(shadow)
...
def _visual_frame(kind: str, desktop: Image.Image, mobile: Image.Image, logo: Image.Image, actions: dict[str, Any]) -> Image.Image:
    base = _gradient(); draw = ImageDraw.Draw(base)
...
def _cta_overlay(actions: dict[str, Any]) -> Image.Image:
    overlay = Image.new("RGBA", (OUT_W, OUT_H), (0,0,0,0)); draw = ImageDraw.Draw(overlay)
    draw.rounded_rectangle((40,165,1040,390), radius=30, fill=(5,25,86,238))
    _draw_centered(draw, actions.get("final_title") or "Scopri il nuovo ECCOMI ONLINE", 225, 50, True, max_width=930)
    draw.rounded_rectangle((110,1570,970,1748), radius=89, fill=(255,255,255,255))
    _draw_centered(draw, actions.get("final_cta") or "Visita eccomionline.com", 1623, 43, True, fill=(5,39,120,255), max_width=790)
    return overlay
...
def _master_segment(ffmpeg: str, master: Path, start: float, duration: float, output: Path, overlay: Path | None = None) -> None:
    if overlay is None:
        ...
        _run([ffmpeg,"-y","-ss",f"{start:.3f}","-i",str(master),"-loop","1","-i",str(overlay),"-t",f"{duration:.3f}","-filter_complex",f"[0:v]scale={OUT_W}:{OUT_H},fps={FPS}[base];[base][1:v]overlay=0:0:format=auto,format=yuv420p[v]","-map","[v]","-an","-c:v","libx264","-preset","veryfast","-crf","18","-movflags","+faststart",str(output)])
...
def _visual_diff_score(ffmpeg: str, before: Path, after: Path, root: Path) -> float:
    ...
    for idx, at in enumerate((2.7,6.0,9.5,13.0)):
        ...
        _sample_frame(ffmpeg, before, at, a); _sample_frame(ffmpeg, after, at, b)
...
def _visual_remix(payload: dict[str, Any], root: Path, master: Path, output: Path, ffmpeg: str, target_duration: float, issues: list[str]):
    urls = {"desktop_asset_url": str(payload.get("desktop_asset_url") or "").strip(), "mobile_asset_url": str(payload.get("mobile_asset_url") or "").strip(), "logo_url": str(payload.get("logo_url") or "").strip()}
    ...
    _download(urls["desktop_asset_url"], dp); _download(urls["mobile_asset_url"], mp); _download(urls["logo_url"], lp)
    ...
    factor = target_duration/sum(durations); durations = [round(x*factor,3) for x in durations]; durations[-1] += target_duration-sum(durations)
    d_png, m_png, b_png, c_png = root/"desktop.png", root/"mobile.png", root/"both.png", root/"cta.png"
    _visual_frame("desktop",desktop,mobile,logo,actions).save(d_png); _visual_frame("mobile",desktop,mobile,logo,actions).save(m_png); _visual_frame("both",desktop,mobile,logo,actions).save(b_png); _cta_overlay(actions).save(c_png)
    segs = [root/f"seg{i}.mp4" for i in range(5)]
    _master_segment(ffmpeg,master,0.0,durations[0],segs[0]); _static_segment(ffmpeg,d_png,durations[1],segs[1]); _static_segment(ffmpeg,m_png,durations[2],segs[2]); _static_segment(ffmpeg,b_png,durations[3],segs[3]); _master_segment(ffmpeg,master,sum(durations[:4]),durations[4],segs[4],c_png)
    visual = root/"visual_only.mp4"; _concat_video(ffmpeg,segs,visual); _attach_original_audio(ffmpeg,visual,master,target_duration,output)
    diff_score = _visual_diff_score(ffmpeg,master,output,root); changed = diff_score >= 0.015
    generation = {"mode":"cpu_visual_compositor_non_gpu","engine":"render_workflows_flex","correction_protocol":VISUAL_PROTOCOL,"timeline_seconds":durations,"scene_count":5,"source_master_url":str(payload.get("master_video_url") or ""),"issues":issues,"correction_note":str(payload.get("correction_note") or ""),"visual_actions":actions,"visual_diff_score":diff_score,"source_sha256":_sha256(master),"output_sha256":_sha256(output)}
    qa = {"technical_pass":True,"output_width":OUT_W,"output_height":OUT_H,"target_duration_seconds":target_duration,"desktop_asset_composited":True,"mobile_asset_composited":True,"logo_composited_deterministically":True,"cta_composited_deterministically":True,"ai_brand_redraw":False,"cuts_only":True,"voice_present":True,"music_present":True,"release_gate_required":True,"cpu_route":True,"audio_preserved":True,"visual_correction_applied":changed,"visual_diff_score":diff_score}
...
def remix_master(ctx: TaskContext, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    evs_code = str(payload.get("evs_code") or "").strip(); job_id = str(payload.get("job_id") or "").strip(); master_url = str(payload.get("master_video_url") or "").strip()
    voice_url = str(payload.get("voice_audio_url") or "").strip(); music_url = str(payload.get("music_audio_url") or "").strip(); output_path = str(payload.get("storage_path") or "").strip(); upload_token = str(payload.get("storage_upload_token") or "").strip(); output_public_url = str(payload.get("output_public_url") or "").strip(); supabase_url = str(payload.get("supabase_url") or "").strip(); anon_key = str(payload.get("supabase_anon_key") or "").strip()
    target_duration = max(8.0,min(30.0,float(payload.get("target_duration_seconds") or 15.0))); voice_volume = max(0.1,min(2.0,float(payload.get("voice_volume") or 1.0))); music_volume = max(0.0,min(1.0,float(payload.get("music_volume") or 0.22)))
    ...
    required = {"evs_code":evs_code,"job_id":job_id,"master_video_url":master_url,"storage_path":output_path,"storage_upload_token":upload_token,"output_public_url":output_public_url,"supabase_url":supabase_url,"supabase_anon_key":anon_key}
    if not visual_mode: required.update({"voice_audio_url":voice_url,"music_audio_url":music_url})
    ...
    with tempfile.TemporaryDirectory(prefix="evs_cpu_") as tmpdir:
        root = Path(tmpdir); master = root/"master.mp4"; output = root/"master_cpu.mp4"; _download(master_url,master); ffmpeg = imageio_ffmpeg.get_ffmpeg_exe()
        if visual_mode:
            generation, qa = _visual_remix(payload,root,master,output,ffmpeg,target_duration,issues)
        else:
            voice, music = root/"voice.wav", root/"music.wav"; _download(voice_url,voice); _download(music_url,music)
            ...
            generation = {"mode":"cpu_compositor_non_gpu","engine":"render_workflows_flex","correction_protocol":str(payload.get("correction_protocol") or "EVS_AUDIO_CORRECTION_V1"),"scene_count":0,"timeline_seconds":[],"source_master_url":master_url,"issues":issues,"voice_volume":voice_volume,"music_volume":music_volume}
            qa = {"technical_pass":True,"output_width":OUT_W,"output_height":OUT_H,"target_duration_seconds":target_duration,"desktop_asset_composited":True,"mobile_asset_composited":True,"logo_composited_deterministically":True,"cta_composited_deterministically":True,"ai_brand_redraw":False,"cuts_only":True,"voice_present":True,"music_present":True,"release_gate_required":True,"cpu_route":True}
        supabase = create_client(supabase_url,anon_key)
        with output.open("rb") as fh: supabase.storage.from_("videos").upload_to_signed_url(path=output_path,token=upload_token,file=fh)
    elapsed = round(time.perf_counter()-started,3); generation["total_seconds"] = elapsed
    _callback(payload,{"event":"evs.video.completed","status":"COMPLETED","job_id":job_id,"spot_url":output_public_url,"customer_reference":evs_code,"generation":generation,"qa":qa})
    return {"ok":True,"evs_code":evs_code,"job_id":job_id,"video_url":output_public_url,"processing_seconds":elapsed,"gpu_started":False,"route":"NON_GPU","correction_protocol":generation.get("correction_protocol"),"visual_diff_score":generation.get("visual_diff_score")}
